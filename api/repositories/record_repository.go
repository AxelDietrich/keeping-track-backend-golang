...
func CreateRecord(db *sql.DB, r *requests.AddRecordRequest, subID int) error {
	...
	rec.Name = r.Name
	rec.Amount = r.Amount
	_, err = tx.Exec("insert into keepingtrack.records (name, amount, subcategory_id) values ($1, $2, $3);",
		rec.Name, rec.Amount, rec.SubcategoryID)
	if err != nil {
		return err
		tx.Rollback()
	}
...
func UpdateRecord(db *sql.DB, recID int, r *requests.AddRecordRequest) error {
	...
	rec := &models.Record{}
	err = tx.QueryRow("select * from keepingtrack.records where id = $1 limit 1;", recID).
...
func PropagateRecordChanges(tx *sql.Tx, r *models.Record) error {
	...
	err = tx.QueryRow("select * from keepingtrack.subcategories where id = $1 limit 1;", r.SubcategoryID).
		Scan(&sub.ID, &sub.Name, &sub.Amount, &sub.CategoryID, &sub.CreatedAt, &sub.UpdatedAt)
	if err != nil {
		return err
	}
	sub.Amount += r.Amount
	_, err = tx.Exec("update keepingtrack.subcategories set amount = $1 where id = $2;",
		sub.Amount, sub.ID)
	if err != nil {
		return err
	}
	categ := &models.Category{}
	err = tx.QueryRow("select * from keepingtrack.categories where id = $1 limit 1;", sub.CategoryID).
		Scan(&categ.ID, &categ.Name, &categ.AccountID, &categ.Income)
	balance := &models.Balance{}
	err = tx.QueryRow("select * from keepingtrack.balances where account_id = $1 limit 1;", categ.AccountID).
	...
	if categ.Income {
		if categ.Name == "Income" {
			if currentAmount+r.Amount >= 0 { //if true, then the value of r.Amount is negative and the deleted income corresponds to spent funds
				...
				err = tx.QueryRow("select s.id, s.name, s.amount, s.category_id, s.created_at, s.updated_at from keepingtrack.subcategories s inner join keepingtrack.categories c on s.category_id = c.id where s.name = 'Auto-generated' and c.account_id = $1 limit 1;", balance.AccountID).
				...
				_, err = tx.Exec("update keepingtrack.subcategories set amount = $1 where id = $2;",
					subAutoDebt.Amount, subAutoDebt.ID)
				if err != nil {
					tx.Rollback()
					return err
				}
				...
				_, err = tx.Exec("insert into keepingtrack.records (name, amount, subcategory_id) values ($1, $2, $3);",
					debtRecord.Name, debtRecord.Amount, debtRecord.SubcategoryID)
				if err != nil {
					tx.Rollback()
					return err
				}
		...
		if currentAmount-r.Amount >= 0 {
			...
			debtRecord.SubcategoryID = subAutoDebt.ID
			_, err = tx.Exec("insert into keepingtrack.debt_records (name, amount, subcategory_id, payed) values ($1, $2, $3, false);",
				debtRecord.Name, debtRecord.Amount, debtRecord.SubcategoryID)
			if err != nil {
				tx.Rollback()
				return err
	...
	_, err = tx.Exec("update keepingtrack.balances set available_amount = $1, savings_amount = $2, debt = $3;",
		balance.AvailableAmount, balance.SavingsAmount, balance.Debt)
	if err != nil {
		tx.Rollback()
		return err
	}
